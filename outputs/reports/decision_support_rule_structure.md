...
- Current first optimization target: Optimize the execution path itself, including test efficiency, parallelization, flake reduction, and execution simplification.
...
- First optimization target rule: The repo maps the latest recommendation style to its profile-derived first optimization target.
- Current first optimization target: Stabilize entry/setup variability and reduce execution-path cost; then inspect the remaining residual tail.
